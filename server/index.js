...
// POST /api/projects/:projectId/files/extract - Extract archive files (zip, tar, tar.gz, tgz, tar.bz2)
app.post('/api/projects/:projectId/files/extract', authenticateToken, async (req, res) => {
    try {
        ...
        const fileNameLower = path.basename(resolvedPath).toLowerCase();

        const isZip = ext === '.zip';
        const isTar = ext === '.tar';
        const isGz = ext === '.gz' || fileNameLower.endsWith('.tar.gz') || fileNameLower.endsWith('.tgz');
        const isBz2 = ext === '.bz2' || fileNameLower.endsWith('.tar.bz2');

        if (!isZip && !isTar && !isGz && !isBz2) {
            return res.status(400).json({ error: 'Unsupported archive format. Supported: zip, tar, tar.gz, tgz, tar.bz2' });
        }

        // Determine extract target directory
        const parentDir = path.dirname(resolvedPath);
        let baseName = path.basename(resolvedPath, ext);

        // Handle .tar.gz and .tgz specially
        if (fileNameLower.endsWith('.tar.gz')) {
            baseName = path.basename(resolvedPath, '.tar.gz');
        } else if (fileNameLower.endsWith('.tgz')) {
            baseName = path.basename(resolvedPath, '.tgz');
        } else if (fileNameLower.endsWith('.tar.bz2')) {
            baseName = path.basename(resolvedPath, '.tar.bz2');
        }

        // Find a unique target directory name
        let targetName = baseName;
        let targetDir = path.join(parentDir, targetName);
        let counter = 1;
        while (fs.existsSync(targetDir)) {
            targetName = `${baseName}_${counter}`;
            targetDir = path.join(parentDir, targetName);
            counter++;
        }

        // Validate target directory is within project
        const targetValidation = validatePathInProject(projectRoot, path.relative(projectRoot, targetDir));
        if (!targetValidation.valid) {
            return res.status(403).json({ error: 'Invalid extract target path' });
        }
        ...
        if (isZip) {
            ...
            // Security check: validate entries for path traversal
            const entries = zip.getEntries();
            for (const entry of entries) {
                const entryName = entry.entryName;
                if (entryName.startsWith('/') || entryName.includes('..')) {
                    return res.status(400).json({ error: 'Unsafe zip file: path traversal detected' });
                }
            }

            zip.extractAllTo(targetDir, true);
        } else if (isTar || isGz || isBz2) {
            ...
            // Add gzip option for .gz files
            if (isGz) {
                extractOptions.gzip = true;
            }

            // Add bzip2 option for .bz2 files
            if (isBz2) {
                extractOptions.gzip = false;
                // tar package doesn't natively support bz2, we'll use a different approach
                const { spawn } = await import('child_process');
                await new Promise((resolve, reject) => {
                    const proc = spawn('tar', ['-xjf', resolvedPath, '-C', targetDir]);
                    proc.on('close', (code) => {
                        if (code === 0) {
                            resolve();
                        } else {
        ...
        res.json({
            success: true,
            extractTo: targetName,
            extractPath: targetDir,
            message: `Extracted to ${targetName}`
        });
    } catch (error) {
...
// GET /api/projects/:projectId/files/open - Open file in browser (for HTML, PDF, etc.)
app.get('/api/projects/:projectId/files/open', authenticateToken, async (req, res) => {
    try {
        const { projectId } = req.params;
        const { path: filePath } = req.query;
